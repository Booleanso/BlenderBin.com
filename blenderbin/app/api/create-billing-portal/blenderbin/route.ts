...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    // Check if the user has a BlenderBin Stripe customer ID
    if (!stripeCustomerId) {
      // No Stripe customer yet, create one for BlenderBin
      try {
        const user = await auth.getUser(uid);
        const email = user.email || 'unknown@example.com';
        
        // Create a new customer for BlenderBin
        ...
        console.error('Error creating BlenderBin Stripe customer:', error);
        return NextResponse.json({ 
          success: false, 
          error: 'Failed to create BlenderBin Stripe customer',
    ...
    console.error('Error creating BlenderBin billing portal session:', error);
    return NextResponse.json({ 
      success: false, 
      error: 'Failed to create BlenderBin billing portal session' 
    }, { status: 500 });
  }
} 